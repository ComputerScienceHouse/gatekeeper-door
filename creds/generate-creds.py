...
for realm in ["DOORS", "MEMBER_PROJECTS", "DRINK"]:
    ...
    print(
        "export GK_REALM_" + realm + "_MOBILE_PRIVATE_KEY=" + shlex.quote(private_key)
    )
    ...
    print(
        "export GK_REALM_"
        + realm
        + "_MOBILE_CRYPT_PRIVATE_KEY="
        + shlex.quote(keyPair.exportKey().decode("ascii"))
    )
    ...
    print(
        "export GK_REALM_"
        + realm
        + "_MOBILE_CRYPT_PUBLIC_KEY="
        + shlex.quote(publicKeyText)
    )
