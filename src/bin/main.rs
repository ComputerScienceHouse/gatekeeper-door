...
fn main() {
    ...
    let access_point = env::var("GK_ACCESS_POINT").unwrap();
    let provisions = Provisions {
        ...
        mqtt_password: env::var("GK_MQTT_PASSWORD").unwrap_or_else(|_| "".to_string()),
        mqtt_username: env::var("GK_MQTT_USERNAME").unwrap_or_else(|_| "".to_string()),
...
fn check_mqtt(
    ...
    let access_denied = provisions.prefix + "/access_denied";
...
fn door_heartbeat(client: mqtt::AsyncClient, provisions: Provisions) {
    let heartbeat = provisions.prefix + "/heartbeat";
    loop {
...
fn run(_sdone: chan::Sender<()>, args: ArgMatches<'_>, provisions: Provisions) {
    ...
    let superusers: HashMap<String, String> = HashMap::new();
    ...
    loop {
        ...
        let realm = Realm::new(
            ...
            &provisions.auth_key,
            &provisions.read_key,
            &provisions.update_key,
            &provisions.public_key,
        ...
        if let Ok(Some(association)) = device.authenticate_tag(&mut realm) {
            ...
            if superusers.contains_key(&association) {
                ...
                // Yay!
                let payload = json!({ "association": association }).to_string();
                let msg = mqtt::Message::new(access_requested.clone(), payload, mqtt::QOS_1);
